...
export default G6 => {
  ...
  G6.registerNode(
    "rect-node",
    {
      ...
      // 当前节点的样式集合
      getShapeStyle(cfg) {
        const width = cfg.style.width || 220;
        const height = cfg.style.height || 46;
  ...
  G6.registerNode(
    "diamond-node",
    {
      ...
      // 返回菱形的路径
      getPath(cfg) {
        const size = cfg.style.size || [160, 160]; // 如果没有 size 时的默认大小
        const width = size[0];
        const height = size[1];
  ...
  G6.registerNode(
    "triangle-node",
    {
      ...
      getPath(cfg) {
        ...
        const size = cfg.style.size || [140, 240]; // 如果没有 size 时的默认大小
        const width = size[0];
        const height = size[1];
